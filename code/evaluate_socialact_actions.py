...
def load_ground_truth(csv_path):
    ...
    with open(csv_path, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            content_raw = row['action_content_answer'].strip()

            true_content_label = content_label_map.get(content_raw, '-')
            ...
            truth[id_val] = {
                ...
                'action_content_answer': true_content_label,
...
def evaluate_file(json_path, ground_truth, verbose=True, max_samples=None):
    ...
    for item in data:
        ...
        true_cont = true_data['action_content_answer']
        ...
        if pred_index is not None:
            if pred_index < 3:
                pred_content_label = pred_label_map[str(pred_index)]
            else:
                pred_content_label = '-'
        else:
        ...
        if is_obj_invalid or is_type_invalid or is_content_invalid:
            if verbose:
                print(f"{id_val:<25} {pred_tup[0]:<12} {true_obj:<12} {'INV':<6} "
                      ...
                      f"{pred_tup[2][:30]:<35} {true_cont[:30]}")
            continue
        ...
        if verbose:
            ...
            true_cont_display = true_cont
            if true_cont in ['content_A','content_B','content_C']:
                idx = {'content_A':0,'content_B':1,'content_C':2}.get(true_cont,0)
                true_cont_display = content_options[idx][:30] if idx < len(content_options) else true_cont
            elif true_cont == '-':
                true_cont_display = '-'
